...
public class CodeAnalyzerUtil {
	 ...
	 *
	 * @param ruleNameList List of rule class names
	 * @return List of rule class instances
	 */
	public static List<JavaCodeRule<TypeElement>> getRuleInstances(Set<String> ruleNameList) {
		List<JavaCodeRule<TypeElement>> classRules = new LinkedList<JavaCodeRule<TypeElement>>();
		try {
			for (String ruleName : ruleNameList) {
				Class classDefinition = Class.forName(ruleName);
